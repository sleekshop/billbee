...
public static function SearchContents($constraint=array(),$left_limit,$right_limit,$order_columns=array(),$order_type="ASC",$lang=DEFAULT_LANGUAGE,$needed_attributes=array())
{
	$sr=new SleekShopRequest();
	$xml=$sr->search_contents($constraint,$left_limit,$right_limit,$order_columns,$order_type,$lang,$needed_attributes);
	$xml=new SimpleXMLElement($xml);
    $result["contents"]=self::get_contents_from_xml($xml);
    $result["count"]=(int)$xml->count;
    return($result);
}
}
